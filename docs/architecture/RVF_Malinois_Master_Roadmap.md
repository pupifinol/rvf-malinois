...
> Last known head at authoring time: commit `db86735` (F4.6F-0 — Historical Trend API Plan). Previously anchored at `51dc626` (F4.6E.1), `22fa2ca` (F4.6E-0), `d35a2b8` (F4.6D.1), `901cd22` (F4.6D-0), `04dadc4` (DX-4), and `1495457` (F4.6B.1).
...
| Historical trend API | Planned — F4.6F-0 plan closed (commit `db86735`); implementation deferred to F4.6F.1. The existing `GET /api/v1/telemetry/trends` (F4.4F, `5e92a13`) will be **hardened / extended** in F4.6F.1 with optional bucketing (`bucket` / `aggregate` / `qualityPolicy` query params) via plain-PostgreSQL `date_bin` (PG 14+, present in the running PG 16 image). **Raw-mode response shape must stay byte-identical** so the F4.5E adapter and existing 5 service tests keep working. **No TimescaleDB-specific feature** is required in F4.6F.1 (ADR-007 §4 keeps Timescale optional); no new migration, no materialized view, no continuous aggregate. Multi-tag reads, materialized projection, frontend per-screen migration, auth, and rate limiting all remain deferred. |
...
| **F4.6F.1** | Historical Trend API Implementation | **Next** | Runtime | Extensions to `apps/backend/src/telemetry/{telemetry.controller,trends.service,contracts/trends}.ts` adding optional bucketing branch + ~10–15 new mocked-Prisma tests | — | Read-side only; uses the existing `telemetry_readings_unit_tag_time_idx` access path. Plan-first deliverable; scope per F4.6F-0 §6 / §7 / §15. Frontend per-screen migration of the Operations chart (off F2 simulator) remains separate (candidate part of F4.5G+). |
...
## 5. Current Implemented Capabilities (as of `51dc626`)
...
- **173/173 backend tests across 15 spec files** as of F4.6E.1 (`51dc626`). Breakdown: 22 ingestion tests (F4.6B.1) + 11 projection tests (F4.6C.1) + 9 ingestion-projection integration tests (F4.6C.1) + 21 alarm evaluation tests (F4.6D.1) + 8 ingestion-alarm integration tests (F4.6D.1) + 1 refined ingestion isolation test (F4.6D.1) + 10 emitter tests (F4.6E.1) + 11 gateway tests (F4.6E.1) + 12 ingestion-realtime integration tests (F4.6E.1) + 69 baseline tests. F4.6E.1 coverage adds: three event kinds emitted to per-tenant rooms; `RVF_REALTIME_EMIT_ENABLED` gate (off / on / mid-test flip); best-effort throw containment; tenant room isolation (no cross-tenant leakage); subscribe / unsubscribe room join/leave with malformed-body rejection; bodyless-unsubscribe leaves every fan-out room while preserving the socket-id room; emit-after-commit call-order assertion (transaction → emit, never reversed); emit NOT invoked on rollback / duplicate / conflict / rejected / projection-rollback / alarm-rollback paths. Refined isolation invariants: ingestion still delegates `prisma.liveReading.*` to `LiveReadingsProjectionService` and `prisma.alarmEvent.*` / `prisma.alarmRule.*` to `AlarmEvaluationService`; ingestion now also delegates all `RealtimeGateway.server.emit` to `RealtimeEmitterService`; no Jobs.
...
Already closed since the previous revision of this roadmap (commit `1495457`): DX-1 (`b19e77a`), DX-2 (`e3ccb52`), DX-3 (`65cb736`), F4.6C-0 (`f126c5c`), F4.6C.1 (`49a8349`), DX-4 (`04dadc4`), F4.6D-0 (`901cd22`), F4.6D.1 (`d35a2b8`), F4.6E-0 (`22fa2ca`), F4.6E.1 (`51dc626`), **F4.6F-0 (`db86735`)**.

The recommended order from here:

1. **F4.6F.1 — Historical Trend API Implementation.** Read-side extension of the existing `GET /api/v1/telemetry/trends`. Scope, query params, bucketing primitives, response shape, limits, and acceptance all locked by F4.6F-0 (`db86735`). Adds optional `bucket` / `aggregate` / `qualityPolicy` query params; runs plain-PostgreSQL `date_bin` aggregation with a `generate_series` LEFT JOIN for the bucket grid; raw-mode shape preserved byte-identical. `TRENDS_BUCKETS_MAX=1500` cap enforced in Zod. No schema / no migration / no frontend change. Expected ~10–15 new backend tests bringing total to ~185–190.
2. **F4.5G — Resume UI adapter wiring** (Wells, Equipment, Catalog, …). Once F4.6F.1 ships, the Operations chart screen migration can wire all three reads together: F4.6F.1 trend reads on mount + on reconnect, candidate F4.6C.2 latest-value reads (if it lands first) for tile widgets, F4.6E.1 realtime push for tail updates. May also proceed in parallel with F4.6F.1 for non-telemetry screens.

**Why F4.6F.1 is next:**

- **F4.6F-0 is closed** (`db86735`). The plan locks every decision the implementation has to make: the 3 new optional query params with their XOR / validation refines (§7), the 5 fixed bucket sizes mapped to `date_bin` (§7.4), the 6 aggregation functions (§6.1), the response shape contract (§7.5), the bucket-count cap (§10.2), the tenant-scoping seam preservation (§11), and the 12 acceptance criteria (§15). F4.6F.1 implements the plan; it does not redesign.
- **The existing `(unit_id, canonical_tag_id, timestamp DESC)` index is sufficient.** F4.6F-0 §5.2 / §10.1 confirms the access path the F4.4F service already uses also covers the bucketed query — no new index, no schema delta.
- **DX-3 (Definition of Done) remains in force.** F4.6F.1 follows the codified plan-first → implementation pattern (F4.2A → F4.2B, F4.6A.0 → F4.6A.1, F4.6B-0 → F4.6B.1, F4.6C-0 → F4.6C.1, F4.6D-0 → F4.6D.1, F4.6E-0 → F4.6E.1).
- **Sequencing F4.6F.1 before F4.5G** ensures any per-screen migration that touches the Operations chart has a real trend endpoint to wire against — rather than authoring a per-screen migration twice (once against mocks, once against the new endpoint).
...
*Master Roadmap, last refreshed at HEAD `db86735` (F4.6F-0 — Historical Trend API Plan). Previous refresh anchors: `51dc626` (F4.6E.1), `22fa2ca` (F4.6E-0), `d35a2b8` (F4.6D.1), `901cd22` (F4.6D-0), `04dadc4` (DX-4). Originally authored at HEAD `1495457` (F4.6B.1). Update on every phase close.*
