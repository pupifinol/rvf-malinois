...
> Last known head at authoring time: commit `51dc626` (F4.6E.1 — WebSocket Fan-out Implementation). Previously anchored at `22fa2ca` (F4.6E-0), `d35a2b8` (F4.6D.1), `901cd22` (F4.6D-0), `04dadc4` (DX-4), and `1495457` (F4.6B.1).
...
| **F4.6F-0** | Historical Trend API / Operations Trend Support Plan | **Next** | Plan | TBD | — | Builds on `telemetry_readings` populated by F4.6B+; now downstream of a stable realtime push (F4.6E.1) so the trend API can be designed against the actual settled state. |
...
Already closed since the previous revision of this roadmap (commit `1495457`): DX-1 (`b19e77a`), DX-2 (`e3ccb52`), DX-3 (`65cb736`), F4.6C-0 (`f126c5c`), F4.6C.1 (`49a8349`), DX-4 (`04dadc4`), F4.6D-0 (`901cd22`), F4.6D.1 (`d35a2b8`), F4.6E-0 (`22fa2ca`), **F4.6E.1 (`51dc626`)**.

The recommended order from here:

1. **F4.6F-0 — Historical Trend API Plan.** Bucketing / downsampling / multi-tag read decisions for `/api/v1/telemetry/trends`. UI cutover plan for Operations charts off the F2 simulator + F3 mock and onto live `telemetry_readings` reads. Decide how the trend API complements vs. duplicates the F4.6E.1 realtime push and any future F4.6C.2 latest-value read API. Plan-first per the DX-3 pattern.
2. **F4.6F.1 — Historical Trend API Implementation.** Endpoint extensions and the Operations chart screen migration.
3. **F4.5G — Resume UI adapter wiring** (Wells, Equipment, Catalog, …). May also proceed in parallel with F4.6F+ for non-telemetry screens; once the trend API ships, the Operations chart screen migration can wire it alongside the F4.6E.1 realtime events as a per-screen migration.

**Why F4.6F-0 is next:**

- **F4.6E.1 is closed** (`51dc626`). The realtime push is now stable end-to-end — backend Socket.IO scaffold, `RealtimeEmitterService`, per-tenant rooms, emit-after-commit semantics, env gate. F4.6F's trend API can be sized against the actual settled realtime contract instead of designing around a moving target.
- **Trend API and realtime push are complementary, not competing.** Realtime push covers "what just happened" (per F4.6E.1 §8 the three event kinds carry the most recent state); the trend API covers "what happened over time" (bucketed history reads). F4.6F-0 must lock how a Operations chart consumes both — typically initial REST trend read on mount, realtime push for tail updates, REST re-read after reconnect.
- **DX-3 (Definition of Done) remains in force.** Plan-first per the codified pattern (F4.2A → F4.2B, F4.6A.0 → F4.6A.1, F4.6B-0 → F4.6B.1, F4.6C-0 → F4.6C.1, F4.6D-0 → F4.6D.1, F4.6E-0 → F4.6E.1). F4.6F-0 follows the same shape: lock scope / semantics / API / tests / acceptance before any code lands.
- **Sequencing F4.6F before F4.5G** ensures any per-screen migration that touches the Operations chart has a real trend endpoint to wire against, rather than authoring a per-screen migration twice (once against mocks, once against the new endpoint).
...
*Master Roadmap, last refreshed at HEAD `51dc626` (F4.6E.1 — WebSocket Fan-out Implementation). Previous refresh anchors: `22fa2ca` (F4.6E-0), `d35a2b8` (F4.6D.1), `901cd22` (F4.6D-0), `04dadc4` (DX-4). Originally authored at HEAD `1495457` (F4.6B.1). Update on every phase close.*
