...
> Last known head at authoring time: commit `946a023` (F4.6F.1 — Historical Trend API Implementation). Previously anchored at `db86735` (F4.6F-0), `51dc626` (F4.6E.1), `22fa2ca` (F4.6E-0), `d35a2b8` (F4.6D.1), `901cd22` (F4.6D-0), `04dadc4` (DX-4), and `1495457` (F4.6B.1).
...
| **F4.5G+** | Per-screen migrations (Wells, Equipment, Catalog, Tags, Settings; Operations chart cutover from F2 simulator) | **Next** | Frontend | Per-screen migration of non-telemetry screens off the F3 mock adapter; Operations chart screen migration consuming F4.6F.1 trend reads + F4.6E.1 realtime push together | — | The F4.6 backend arc is complete (ingest → projection → alarm → realtime push → bucketed trend reads). F4.5G can now wire screens against real backend surfaces instead of mock-only. |
...
## 5. Current Implemented Capabilities (as of `946a023`)
...
- **Operations chart cutover from F2 simulator + F3 mock to live `telemetry_readings` reads.** Frontend per-screen migration owned by **F4.5G+**. The F4.5E backend-adapter seam already exists; the chart screen needs to be wired through it (and through F4.6E.1 realtime push for tail updates).
- **Frontend per-screen wiring of realtime push** (`telemetry.reading.accepted` / `live_reading.updated` / `alarm.event.created`). F4.6E.1 ships server-side emission; consumer migration is the same F4.5G+ task as the chart cutover.
...
Already closed since the previous revision of this roadmap (commit `1495457`): DX-1 (`b19e77a`), DX-2 (`e3ccb52`), DX-3 (`65cb736`), F4.6C-0 (`f126c5c`), F4.6C.1 (`49a8349`), DX-4 (`04dadc4`), F4.6D-0 (`901cd22`), F4.6D.1 (`d35a2b8`), F4.6E-0 (`22fa2ca`), F4.6E.1 (`51dc626`), F4.6F-0 (`db86735`), **F4.6F.1 (`946a023`)**.
...
1. **F4.5G — Resume UI adapter wiring.** First per-screen migration target is the Operations chart cutover from the F2 simulator + F3 mock to live backend surfaces: F4.6F.1 trend reads on mount + on reconnect (raw mode for high-fidelity windows; bucketed mode for compressed history); F4.6E.1 realtime push (`telemetry.reading.accepted` / `live_reading.updated` / `alarm.event.created`) for tail updates; candidate F4.6C.2 latest-value reads (when it lands) for tile widgets. Non-telemetry screens (Wells, Equipment, Catalog, Tags, Settings) can proceed in parallel from the existing F4.5B / F4.5C adapter base.
2. **(Candidate) F4.6C.2 — Latest-value Read API.** Public `GET /api/v1/telemetry/latest` (or equivalent) over the now-populated `live_readings`. Sized when the first screen migration needs it. Independent of F4.5G ordering.

**Why F4.5G is next:**

- **The backend telemetry arc is settled.** F4.6F.1 closed the historical-read primitive; F4.6E.1 closed the realtime push; F4.6D.1 / F4.6C.1 / F4.6B.1 closed the write path. There is no remaining backend phase that an Operations chart migration would have to wait on.
- **The chart cutover is a per-screen migration**, not a backend phase — it's exactly what the F4.5G+ slot has been holding open since the F4.5F units selector migration shipped.
- **DX-3 (Definition of Done) remains in force** for screen migrations as well: each per-screen migration is its own small slice with its own closeout, exactly as F4.5F demonstrated.
- **Sequencing F4.5G before any candidate F4.6F.2 / F4.6F.3 / F4.6F.4** lets real screen-consumption needs drive the priority of the candidate trend-track follow-ups: if a chart needs multi-tag in one call, F4.6F.2 promotes; if profiling shows the live aggregate is too slow under realistic load, F4.6F.4 promotes; etc.
...
| Frontend Operations chart mock drift vs. live backend reads. | The Operations chart still renders from the F2 simulator + F3 mock today. With F4.6F.1 (trend reads) + F4.6E.1 (realtime push) both live, the gap between mock-rendered visuals and what real backend reads would produce can widen silently. Mitigation is the F4.5G+ chart cutover; until that lands, treat the Operations chart as illustrative, not authoritative, when validating backend behavior. Reviewer rejects any "looks right on the chart" claim about backend correctness until the chart is wired through the F4.5E adapter to real endpoints. |
...
*Master Roadmap, last refreshed at HEAD `946a023` (F4.6F.1 — Historical Trend API Implementation). Previous refresh anchors: `db86735` (F4.6F-0), `51dc626` (F4.6E.1), `22fa2ca` (F4.6E-0), `d35a2b8` (F4.6D.1), `901cd22` (F4.6D-0), `04dadc4` (DX-4). Originally authored at HEAD `1495457` (F4.6B.1). Update on every phase close.*
